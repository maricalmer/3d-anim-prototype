...
function ZeusModel({ position, scale, rotation, scrollYProgress }) {
  ...
  const { scene } = useGLTF("./3dmodels/studio_cube.glb");
  useFrame((state, delta) => {
    ...
    state.scene.position.set(0, -2 * (scrollYProgress.current * 0.3), 2);
    state.camera.position.set(0, -2, 5 - scrollYProgress.current * 1);
    state.camera.lookAt(0, 0, 0);
  });
  return (
    <>
